...
/******************************************************************************
* Function Name : LCD_DrawScacchiera
* delta = dim square
* spaceSquares = distance between two squares
* color = color Scacchiera
*******************************************************************************/


void LCD_DrawScacchiera(uint16_t delta, uint16_t spaceSquares, uint16_t color){
	int j, i;
	...
	for(i = 0; i < 7; i++){
		...
		for(j = 0; j < 7; j++){
				...
				LCD_DrawLine(x, y, x+delta, y, color); //lato alto
				LCD_DrawLine(x, y+delta, x+delta, y+delta, color); //lato basso
			
				LCD_DrawLine(x, y, x, y+delta, color); //lato laterale sinistra
				LCD_DrawLine(x+delta, y, x+delta, y+delta, color); //lato laterale destra
			
				x = x + delta + spaceSquares;
		}
		y = y + delta + spaceSquares;
	}
	...
	drawPlayer(1, 0, 6);
	//insert moves player
	insertPlayerMoves(12, 6, 2);
	insertPlayerMoves(0, 6, 1);
}
...
/******************************************************************************
* Function Name : LCD_DrawInformation
*******************************************************************************/

char temp[3];

void LCD_DrawInformation(uint16_t width, uint16_t lenght, uint16_t color){
	...
	//P1:walls
	LCD_DrawLine(x, y, x, y+lenght, color); //linea verticale sinistra
	LCD_DrawLine(x, y, x+width, y, color); //linea alta
	LCD_DrawLine(x+width, y, x+width, y+lenght, color); //linea laterale destra
	LCD_DrawLine(x, y+lenght, x+width, y+lenght, color); //linea bassa
	
	GUI_Text(18, 260, (uint8_t *) "P1 Wall", White, Black);
	PutChar(44, 280, '0'+wallsPlayer1, White, Black);
	
	//timer
	LCD_DrawLine(x+width+delta, y, x+width+delta, y+lenght, color); //linea verticale sinistra
	LCD_DrawLine(x+width+delta, y, x+2*width+delta, y, color); //linea alta
	LCD_DrawLine(x+2*width+delta, y, x+2*width+delta, y+lenght, color); //linea laterale destra
	LCD_DrawLine(x+width+delta, y+lenght, x+2*width+delta, y+lenght, color); //linea bassa

	drawSecondsOnDislay();
	
	//P2:walls
	LCD_DrawLine(x+2*width+2*delta, y, x+2*width+2*delta, y+lenght, color); //linea verticale sinistra
	LCD_DrawLine(x+2*width+2*delta, y, x+3*width+2*delta, y, color); //linea alta
	LCD_DrawLine(x+3*width+2*delta, y, x+3*width+2*delta, y+lenght, color); //linea laterale destra
	LCD_DrawLine(x+2*width+2*delta, y+lenght, x+3*width+2*delta, y+lenght, color); //linea bassa
	
	GUI_Text(18+2*width+2*delta, 260, (uint8_t *) "P2 Wall", White, Black);
	PutChar(44+2*width+2*delta, 280, '0'+wallsPlayer2, White, Black);
